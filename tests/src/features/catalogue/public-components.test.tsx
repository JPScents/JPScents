...
describe("public catalogue controls", () => {
  ...
  it("uses friendly preference labels and exposes the results announcement and no-match recovery", () => {
    render(<HelpMeChoose initial={{ scentCharacters: ["FRESH"], occasions: ["DATE_NIGHT"], timeOfDay: "NIGHT" }} submitted results={[]} />);
    ...
    expect(screen.getByRole("link", { name: "Browse all perfumes" })).toHaveAttribute("href", "/perfumes");
  ...
  it("renders a controlled bottle fallback when a product image is unavailable", () => {
    ...
    expect(screen.getByRole("link", { name: /Quiet Fig/ })).toHaveAttribute("href", "/perfume/quiet-fig");
  });
});
