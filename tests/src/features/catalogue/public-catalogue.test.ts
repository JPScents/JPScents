...
describe("public catalogue URL parsing", () => {
  ...
  it("preserves valid multi-select preferences and ignores invalid values and Either", () => {
    expect(parsePreferences({ scent: "FRESH,INVALID,WOODY", occasion: ["WORK", "INVALID", "DATE_NIGHT"], time: "EITHER" })).toEqual({ scentCharacters: ["FRESH", "WOODY"], occasions: ["WORK", "DATE_NIGHT"], timeOfDay: undefined });
  });

  it("uses published-only query constraints and availability-first ordering", async () => {
    vi.mocked(prisma.perfume.findMany).mockResolvedValueOnce([perfume({ id: "unavailable", variants: [{ id: "v", sizeValue: 30, priceMinor: 120000, quantity: 0 }] }), perfume()]);
  ...
  it("serializes only a published detail and preserves ordered variants", async () => {
    vi.mocked(prisma.perfume.findFirst).mockResolvedValueOnce(perfume({ variants: [{ id: "small", sizeValue: 30, priceMinor: 100000, quantity: 1 }, { id: "large", sizeValue: 50, priceMinor: 150000, quantity: 0 }] }));
    const detail = await getPerfumeBySlug("quiet-fig");
    expect(prisma.perfume.findFirst).toHaveBeenCalledWith(expect.objectContaining({ where: { slug: "quiet-fig", status: "PUBLISHED" } }));
    expect(detail?.variants).toEqual(expect.arrayContaining([expect.objectContaining({ id: "small", sizeLabel: "30 mL", isAvailable: true }), expect.objectContaining({ id: "large", isAvailable: false })]));
  });

  it("excludes the current perfume when ranking related available perfumes", async () => {
    vi.mocked(prisma.perfume.findMany).mockResolvedValueOnce([perfume({ id: "related-a", name: "Amber", scentCharacters: ["FRESH", "WARM"] }), perfume({ id: "related-b", name: "Cedar", scentCharacters: ["FRESH"] })]);
    const related = await getRelatedPerfumes({ id: "p-1", slug: "quiet-fig", name: "Quiet Fig", scentCue: "Soft fig", scentCharacters: ["FRESH"], occasions: ["EVERYDAY"], timesOfDay: ["DAY"], description: "A quiet perfume.", primaryImageAlt: "", startingPrice: "From ₦1,200", isAvailable: true, variants: [] });
    expect(prisma.perfume.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ status: "PUBLISHED", id: { not: "p-1" }, variants: { some: { quantity: { gt: 0 } } } }) }));
  ...
  it("ranks available recommendations with plain-language reasons and labels summaries", async () => {
    vi.mocked(prisma.perfume.findMany).mockResolvedValueOnce([perfume({ id: "best-match", scentCharacters: ["FRESH", "WOODY"], occasions: ["WORK"] }), perfume({ id: "less-match", name: "Less", scentCharacters: ["FRESH"], occasions: ["EVERYDAY"] })]);
    const results = await recommendPerfumes({ scentCharacters: ["FRESH", "WOODY"], occasions: ["WORK"], timeOfDay: undefined });
    expect(results.map((item) => item.id)).toEqual(["best-match", "less-match"]);
    expect(results[0]?.matchReason).toContain("Fresh, Woody, Work");
    expect(preferenceSummary({ scentCharacters: ["FRESH"], occasions: ["DATE_NIGHT"], timeOfDay: "NIGHT" })).toEqual(["Fresh", "Evening", "Night"]);
  });
});
