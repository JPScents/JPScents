      ...
      <div className="mt-5 grid gap-5 md:grid-cols-[11rem_1fr]">
        <div className="flex min-h-48 items-center justify-center border border-dashed bg-jp-stone p-3">
          {image?.signedUrl ? (
            ...
            <span className="text-center text-sm text-jp-text-secondary">No primary image</span>
          )}
        </div>
        <div>
          <form action={action} className="grid gap-3">
            ...
            <label className="grid gap-1 text-sm font-medium">
              Image file
              <span className="font-normal text-jp-text-secondary">JPEG, PNG, or WebP</span>
              <input
                required
                name="image"
                type="file"
          ...
          {state.ok ? (
            <p className="mt-3 text-sm text-jp-status-confirmed-foreground">Image saved.</p>
          ) : null}
        </div>
      </div>
    </section>
