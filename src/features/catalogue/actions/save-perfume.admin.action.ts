...
function stagedImage(formData: FormData) {
  ...
  const altText =
    typeof formData.get("primaryImageAlt") === "string"
      ? String(formData.get("primaryImageAlt")).trim()
      : "";
  if (!(file instanceof File) || file.size === 0) return { file: null, altText };
  if (!allowedImageTypes.has(file.type) || file.size > 5 * 1024 * 1024)
    return { file: null, altText, error: "Use JPEG, PNG, or WebP up to 5 MiB." };
  if (!altText) return { file: null, altText, error: "Useful alt text is required." };
  return { file, altText };
}
export async function savePerfume(
  _: PerfumeActionState,
  formData: FormData,
): Promise<PerfumeActionState> {
  ...
  const existing =
    typeof id === "string" && id
      ...
      : null;
  if (typeof id === "string" && id && !existing)
    return { message: "This perfume no longer exists." };
  const conflict = await prisma.perfume.findUnique({ where: { slug: input.slug } });
  if (conflict && conflict.id !== existing?.id)
    return { errors: { slug: "That slug is already in use." } };
  const staged = parseStagedVariants(formData.get("stagedVariants"));
  if (staged.error) return { errors: { variants: staged.error } };
  ...
  if (Object.keys(publish).length) return { errors: publish };
  if (existing?.isBestseller && input.status !== "PUBLISHED")
    return { message: "Replace or clear the active Bestseller before unpublishing it." };
  if (existing) {
    await prisma.perfume.update({ where: { id: existing.id }, data: input });
    revalidatePath("/admin/perfumes");
    redirect(`/admin/perfumes/${existing.id}`);
  }
  ...
  if (image.file) {
    ...
    const upload = await supabase.storage
      .from("perfume-images")
      .upload(uploadedPath, image.file, { contentType: image.file.type, upsert: false });
    if (upload.error)
      return { message: "Image upload failed. No perfume was created; please try again." };
  }
  try {
    await prisma.perfume.create({
      data: {
        ...
        ...input,
        images: uploadedPath
          ? { create: { path: uploadedPath, altText: image.altText, position: 0 } }
          : undefined,
        variants: staged.variants.length
          ? { create: staged.variants.map((variant) => ({ ...variant, sizeUnit: "ML" })) }
          : undefined,
      },
    });
  } catch (error) {
    if (uploadedPath && supabase)
      await supabase.storage.from("perfume-images").remove([uploadedPath]);
    const code =
      typeof error === "object" && error && "code" in error
        ? (error as { code?: string }).code
        : undefined;
    if (code === "P2002")
      return { errors: { slug: "That slug was just claimed. Choose another stable slug." } };
