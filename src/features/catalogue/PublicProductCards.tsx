...
export function GalleryProductCard({ perfume }: { perfume: PublicPerfumeCard }) {
  return <Link href={siteConfig.routes.perfume(perfume.slug)} className="group block focus-visible:outline-2 focus-visible:outline-offset-4"><article className="border bg-jp-surface p-3"><div className="relative aspect-[4/5] overflow-hidden bg-jp-stone"><ProductImage perfume={perfume} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.02]" />{perfume.label && <span className="absolute left-3 top-3 bg-jp-surface px-2 py-1 text-xs uppercase tracking-[0.14em]">{perfume.label}</span>}{!perfume.isAvailable && <span className="absolute bottom-3 left-3 bg-jp-text-primary px-2 py-1 text-xs text-jp-surface">Unavailable</span>}</div><div className="px-1 pb-2 pt-5"><CardMeta perfume={perfume} /></div></article></Link>;
}
