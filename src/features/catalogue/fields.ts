...
const enumValues = <T extends string>(values: FormDataEntryValue[], options: readonly T[]) =>
  values.filter(
    (value): value is T =>
      typeof value === "string" && (options as readonly string[]).includes(value),
  );
export function parsePerfumeInput(formData: FormData): {
  input?: PerfumeInput;
  ...
  const input = {
    ...
    scentCue: text(formData.get("scentCue")),
    description: text(formData.get("description")),
    status: formData.get("status") === "PUBLISHED" ? "PUBLISHED" : "DRAFT",
    isFeatured: formData.get("isFeatured") === "on",
    scentCharacters: enumValues(formData.getAll("scentCharacters"), scentCharacters),
    occasions: enumValues(formData.getAll("occasions"), occasions),
    timesOfDay: enumValues(formData.getAll("timesOfDay"), timesOfDay),
  } as PerfumeInput;
...
export function parseVariantInput(formData: FormData): {
  input?: VariantInput;
  errors: Record<string, string>;
} {
  const sizeValue = text(formData.get("sizeValue"));
  const priceMinor = parseNairaToMinor(text(formData.get("price")));
...
export function parseStagedVariants(value: FormDataEntryValue | null): {
  ...
  try {
    ...
    const variants = raw.map((item) => {
      ...
      form.set("sizeValue", String(candidate.sizeValue ?? ""));
      form.set("price", String(candidate.price ?? ""));
      form.set("quantity", String(candidate.quantity ?? ""));
      const parsed = parseVariantInput(form);
