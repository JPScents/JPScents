...
import { resolveCartItems, type CartRequestLine, type ResolvedCartLine } from "./cart-resolver.server";
...
function getStorage() { try { return window.localStorage; } catch { return undefined; } }
function readStoredCart() { try { return getStorage()?.getItem(STORAGE_KEY) ?? null; } catch { return null; } }
function writeStoredCart(items: CartRequestLine[]) { try { getStorage()?.setItem(STORAGE_KEY, JSON.stringify({ version: CART_VERSION, items })); } catch { /* Cart remains usable in memory. */ } }

export function parseCartPayload(value: string | null): CartPayload {
  if (!value) return { version: CART_VERSION, items: [] };
  try {
    const parsed: unknown = JSON.parse(value);
    if (!parsed || typeof parsed !== "object" || (parsed as { version?: unknown }).version !== CART_VERSION || !Array.isArray((parsed as { items?: unknown }).items)) return { version: CART_VERSION, items: [] };
    ...
    const items = (parsed as { items: unknown[] }).items.flatMap((item) => {
      ...
      if (typeof perfumeVariantId !== "string" || !UUID_PATTERN.test(perfumeVariantId) || !Number.isSafeInteger(quantity) || !quantity || quantity < 0 || seen.has(perfumeVariantId)) return [];
      seen.add(perfumeVariantId);
      return [{ perfumeVariantId, quantity }];
    });
    return { version: CART_VERSION, items };
  } catch { return { version: CART_VERSION, items: [] }; }
}
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { queueMicrotask(() => setItems(parseCartPayload(readStoredCart()).items)); }, []);
  useEffect(() => {
    const sync = (event: StorageEvent) => { if (event.key === STORAGE_KEY) setItems(parseCartPayload(event.newValue).items); };
    window.addEventListener("storage", sync); return () => window.removeEventListener("storage", sync);
  }, []);
  useEffect(() => {
    let current = true;
    if (!items.length) { queueMicrotask(() => { if (current) { setLines([]); setResolutionState("idle"); } }); return () => { current = false; }; }
    queueMicrotask(() => { if (current) setResolutionState("resolving"); });
    resolveCartItems(items).then((next) => { if (current) { setLines(next); setResolutionState("idle"); } }).catch(() => { if (current) setResolutionState("error"); });
    return () => { current = false; };
  }, [items]);
  const addItem = useCallback((perfumeVariantId: string, quantity: number, maximumQuantity?: number) => {
    if (!perfumeVariantId || !Number.isSafeInteger(quantity) || quantity < 1) return;
    const cap = Number.isSafeInteger(maximumQuantity) && maximumQuantity! > 0 ? maximumQuantity : undefined;
    persist(items.some((item) => item.perfumeVariantId === perfumeVariantId) ? items.map((item) => item.perfumeVariantId === perfumeVariantId ? { ...item, quantity: cap ? Math.min(item.quantity + quantity, cap) : item.quantity + quantity } : item) : [...items, { perfumeVariantId, quantity: cap ? Math.min(quantity, cap) : quantity }]);
    setOpen(true);
  }, [items, persist]);
