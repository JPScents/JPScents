...
export function AdminShell({ children, email }: { children: ReactNode; email: string | null }) {
  ...
  return <Dialog><div className="min-h-screen bg-jp-admin-canvas text-jp-text-primary lg:grid lg:grid-cols-[15.5rem_1fr]">
    ...
    <div><header className="flex h-header-mobile items-center justify-between border-b bg-jp-admin-surface px-5 lg:h-20 lg:justify-end lg:px-admin-content"><DialogTrigger asChild><button className="inline-flex size-11 items-center justify-center lg:hidden" type="button" aria-label="Open admin menu"><Menu className="size-5" aria-hidden="true" /></button></DialogTrigger><span className="text-sm text-jp-text-secondary">{email ?? "Admin"}</span></header><DialogContent animation="from-left" className="inset-y-0 left-0 h-full w-[min(20rem,calc(100%-2.5rem))] max-w-none translate-x-0 translate-y-0 border-y-0 border-l-0 bg-jp-admin-sidebar text-sidebar-foreground lg:hidden"><DialogTitle className="text-sidebar-foreground">Admin navigation</DialogTitle><DialogDescription className="sr-only">Choose an Admin workspace page.</DialogDescription><DialogCloseButton className="text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground" /><div className="mt-10">{nav}</div></DialogContent><main className="p-5 lg:p-admin-content">{children}</main></div>
  </div></Dialog>;
}
